...
export async function renderAdminDashboard() {
  ...
  const exportCsvBtn = document.getElementById('btn-export-csv-ledger');
  if (exportCsvBtn) {
    exportCsvBtn.addEventListener('click', () => {
      ...
      const url = (window.apiBaseUrl || '') + '/api/reports/attendance-csv';
      window.open(url + '?token=' + encodeURIComponent(token), '_blank');
    });
  }

  const auditLogBtn = document.getElementById('btn-view-audit-logs');
  if (auditLogBtn) {
    auditLogBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await fetch((window.apiBaseUrl || '') + '/api/audit-logs', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const logs = res.ok ? await res.json() : [];
        showAuditLogModal(logs);
      } catch (err) {
        showToastNotification('Failed to retrieve security audit logs', 'error');
      }
    });
  }

  function showAuditLogModal(logs) {
